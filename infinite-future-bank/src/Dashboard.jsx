...
export default function Dashboard({ session, onSignOut }) {
  ...
  const [tourAudioEnabled, setTourAudioEnabled] = useState(true);
  ...
  // ==========================================
  // 🔥 TOUR TTS AUDIO ENGINE
  // ==========================================
  useEffect(() => {
    if (!runTour || !tourAudioEnabled) {
    ...
    const fetchTourVoice = async () => {
      if (tourAudioRef.current) tourAudioRef.current.pause();
      
      const content = TOUR_CONTENT[tourStepIndex][tourLanguage];
      const rawText = `${content.title}... ${content.body}`;

      try {
        const { data, error } = await supabase.functions.invoke('generate-speech', {
          body: { text: rawText, languageCode: tourLanguage === 'es' ? 'es-ES' : 'en-US' }
        });
    ...
    fetchTourVoice();
  }, [tourStepIndex, tourLanguage, runTour, tourAudioEnabled]);
  ...
  const handleJoyrideCallback = async (data) => {
    ...
    if (finishedStatuses.includes(status)) {
      ...
      try {
        ...
        setProfile(prev => ({...prev, has_completed_tour: true}));
      } catch(err) {
  ...
  // ==========================================
  // 🔥 CUSTOM BILINGUAL TOUR TOOLTIP
  // ==========================================
  const CustomTourTooltip = ({ index, backProps, closeProps, primaryProps, isLastStep, tooltipProps }) => {
      ...
      <div {...tooltipProps} className="bg-slate-900 text-white p-6 rounded-3xl w-80 shadow-2xl border border-slate-700 font-sans">
        <div className="flex justify-between items-center mb-4 border-b border-slate-700 pb-3">
           ...
           <button onClick={() => setTourAudioEnabled(!tourAudioEnabled)} className="text-slate-400 hover:text-white transition-colors" title="Toggle Voice">
             {tourAudioEnabled ? <Volume2 size={16}/> : <VolumeX size={16}/>}
           </button>
        </div>
        ...
        <main className="flex-1 flex flex-col relative overflow-hidden">
          <header className="border-b border-slate-200/50 bg-white/40 backdrop-blur-xl flex items-center justify-between px-6 z-[100] sticky top-0 pt-[env(safe-area-inset-top)] min-h-[calc(5rem+env(safe-area-inset-top))]">
            ...
            <div className="flex items-center gap-4 md:gap-6 relative">
              ...
              {/* THE APP DRAWER ICON WITH TOUR ID */}
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-100 mt-[env(safe-area-inset-top)] mb-[env(safe-area-inset-bottom)] max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={async (e) => {
              ...
              // 🔥 THE NEW SEND LOGIC (INCLUDES AFR BRIDGE)
              if (activeModal === 'SEND') {
                const amount = parseFloat(e.target.amount.value);
                ...
                if (asset === 'AFR') {
                  ...
                  if (!receiverAddress) {
                    setNotification({ type: 'error', text: 'Enter a valid AFR wallet address.' });
                    setIsLoading(false); return;
                  }
                  ...
                  try {
                    ...
                    const { data, error } = await supabase.functions.invoke('process-afr-transfer', {
                      body: { receiver_address: receiverAddress, amount: amount }
                    });

                    if (error) throw new Error("Network Bridge Error: " + error.message);
                    if (data?.error) throw new Error("Ledger Rejected: " + data.error);

                    setNotification({ type: 'success', text: `AFR Settled on Mainnet. Hash: ${data.tx_hash?.slice(0, 10)}...` });
                    await fetchAllData();
                    setActiveModal(null);
                  } catch (err) {
