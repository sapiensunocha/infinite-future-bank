-- ============================================================
-- VentureX Seed Data: ~80 real African startups + deals + milestones
-- + pitch_video_url column + updated feed RPC
-- ============================================================

-- ── 1. Add pitch_video_url to companies ────────────────────────────────────────
ALTER TABLE public.venturex_companies ADD COLUMN IF NOT EXISTS pitch_video_url TEXT;
ALTER TABLE public.venturex_companies ADD COLUMN IF NOT EXISTS description TEXT;

-- ── 2. Update get_venturex_feed to return pitch_video_url ──────────────────────
DROP FUNCTION IF EXISTS public.get_venturex_feed(INT, INT, TEXT, TEXT);
...
AS $$
  SELECT * FROM (
    ...
    SELECT
      d.id                                   AS event_id,
      ('deal_' || d.status)::TEXT            AS event_type,
      ...
      vc.pitch_video_url                     AS pitch_video_url,
      vc.tagline                             AS tagline
    ...
    SELECT
      m.id, 'milestone_hit',
      vc.legal_name, vc.sector, vc.country,
      NULL, NULL, m.title, m.completed_at,
      vc.pitch_video_url, vc.tagline
    FROM  public.venturex_milestones m
    JOIN  public.venturex_deals      d  ON d.id  = m.deal_id
    JOIN  public.venturex_companies  vc ON vc.id = d.company_id
    WHERE m.is_completed = TRUE AND m.completed_at IS NOT NULL
    ...
    SELECT
      ...
      CASE
        ...
        ELSE '$25M+'
      END,
      vc.equity_offered, NULL, vc.created_at,
      vc.pitch_video_url, vc.tagline
    FROM  public.venturex_companies vc
    WHERE vc.status = 'active' AND vc.is_public = TRUE
      AND (p_sector IS NULL OR vc.sector = p_sector)
...
-- ── 3. Seed investors + companies + deals ─────────────────────────────────────
DO $$
DECLARE
  v_uid  UUID;
...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM public.venturex_companies) > 10 THEN
    RAISE NOTICE 'VentureX already seeded — skipping'; RETURN;
  END IF;
  ...
  (v_uid,'Djamo','FinTech','Neobank','Côte d''Ivoire','https://djamo.com',
   ...
   'https://www.youtube.com/results?search_query=Djamo+Côte+Ivoire+fintech','active',true,true,true,false,now()-interval '12 months'),
  ...
  (v_uid,'Prembly (Identitypass)','CyberSecurity','Identity Verification','Nigeria','https://prembly.com',
  ...
  (v_uid,'54gene','HealthTech','Genomics','Nigeria','https://54gene.com',
   ...
   '54gene is building Africa''s most comprehensive genomics biobank to accelerate drug discovery and personalised medicine for African populations.',
  ...
  -- ── Investors (already inserted above, now get IDs for deals) ─────────────
  -- v_inv1..v_inv5 already set above
  ...
  FROM venturex_companies vc WHERE vc.legal_name = 'Prembly (Identitypass)';
  ...
  -- ── Milestones for signed/closed deals ────────────────────────────────────
  ...
  INSERT INTO public.venturex_milestones (deal_id, title, description, is_completed, is_verified, completed_at)
  SELECT d.id, '7M Farmers Served', 'ALX Africa trained and placed 100,000 tech professionals', true, true, now()-interval '10 months'
  ...
  INSERT INTO public.venturex_milestones (deal_id, title, description, is_completed, is_verified, completed_at)
  SELECT d.id, 'Acquisition — Vezeeta', 'Vezeeta acquired competitor to expand MENA footprint', true, true, now()-interval '11 months'
  ...
  RAISE NOTICE 'VentureX seed complete: companies, investors, deals, milestones inserted.';
END $$;
