    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-8 animate-in fade-in zoom-in-95 duration-500">
      <div className="md:col-span-1 space-y-2 bg-white/60 backdrop-blur-xl border border-white/40 p-4 rounded-3xl shadow-sm h-fit">
        {[
          ...
          { id: 'ACCESSIBILITY', label: 'Accessibility & Display', icon: <Accessibility size={18} /> },
      ...
      <div className="md:col-span-3 bg-white/60 backdrop-blur-xl border border-white/40 p-8 rounded-3xl shadow-sm">
        ...
        {subTab === 'PROFILE' && (
          <div className="space-y-8 max-w-2xl">
            ...
            <div className="bg-slate-50 border border-slate-200 p-8 rounded-3xl mt-8 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-black text-slate-800 flex items-center gap-2"><ShieldCheck className="text-emerald-500" size={20}/> Identity Verification (KYC)</h3>
                {profile?.kyc_status === 'verified' && <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-200/50">Verified</span>}
              </div>
              <p className="text-xs text-slate-500 mb-8 leading-relaxed">Federal regulations require identity verification for Tier-1 liquidity access, Alpha Marketplace investments, and external Stripe payouts.</p>
              <form onSubmit={handleSubmitKYC} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input type="text" value={kycForm.legalName} onChange={(e) => setKycForm({...kycForm, legalName: e.target.value})} disabled={profile?.kyc_status === 'verified'} required className="w-full bg-white border border-slate-200 rounded-xl p-4 font-bold text-sm text-slate-800 outline-none focus:border-blue-500 disabled:opacity-50" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">Date of Birth</label>
                    <input type="date" value={kycForm.dob} onChange={(e) => setKycForm({...kycForm, dob: e.target.value})} disabled={profile?.kyc_status === 'verified'} required className="w-full bg-white border border-slate-200 rounded-xl p-4 font-bold text-sm text-slate-800 outline-none focus:border-blue-500 disabled:opacity-50" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">Phone Number</label>
                    <input type="tel" value={kycForm.phone} onChange={(e) => setKycForm({...kycForm, phone: e.target.value})} disabled={profile?.kyc_status === 'verified'} required className="w-full bg-white border border-slate-200 rounded-xl p-4 font-bold text-sm text-slate-800 outline-none focus:border-blue-500 disabled:opacity-50" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2">Residential Address</label>
                    <input type="text" value={kycForm.address} onChange={(e) => setKycForm({...kycForm, address: e.target.value})} disabled={profile?.kyc_status === 'verified'} required className="w-full bg-white border border-slate-200 rounded-xl p-4 font-bold text-sm text-slate-800 outline-none focus:border-blue-500 disabled:opacity-50" />
                  </div>
                </div>
                {profile?.kyc_status !== 'verified' && (
                  <button type="submit" disabled={isSubmittingKyc} className="w-full mt-4 bg-blue-700 text-white font-black text-[10px] uppercase tracking-widest py-4 rounded-xl shadow-lg hover:bg-blue-600 transition-all flex items-center justify-center gap-2">
            ...
            <div className="bg-slate-50 border border-slate-200 p-8 rounded-3xl mt-8 shadow-sm">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-black text-slate-800">Master Service Agreement & Licensing</h3>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div className="bg-white border border-slate-200 p-5 rounded-2xl text-center shadow-sm">
                  ...
                  <p className="text-xs font-black text-slate-800">EIN: 33-1869013</p>
                </div>
                <div className="bg-white border border-slate-200 p-5 rounded-2xl text-center shadow-sm">
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1">Austria</p>
                  <p className="text-xs font-black text-slate-800">Steuernummer: 91 323/2005</p>
                </div>
                <div className="bg-white border border-slate-200 p-5 rounded-2xl text-center shadow-sm">
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1">Canada</p>
                  <p className="text-xs font-black text-slate-800">CRA: 721487825 RC 0001</p>
                </div>
              </div>
              {!profile?.docs_signed ? (
                <button onClick={handleSignAgreements} disabled={isLoading} className="w-full py-4 bg-blue-700 text-white font-black text-[10px] uppercase tracking-widest rounded-xl shadow-lg hover:bg-blue-600 transition-all flex items-center justify-center gap-2">
                  {isLoading ? <Loader2 className="animate-spin" size={16}/> : 'Cryptographically Sign & Accept Terms'}
                </button>
              ) : (
                <div className="w-full py-4 bg-emerald-50 border border-emerald-200 text-emerald-500 font-black text-[10px] uppercase tracking-widest rounded-xl flex items-center justify-center gap-2 shadow-sm">
            ...
            {linkedCards.length > 0 ? (
              <div className="grid gap-4">
                {linkedCards.map(card => (
                  <div key={card.id} className="bg-white border border-slate-200 rounded-2xl p-5 flex items-center justify-between shadow-sm group hover:border-slate-300 transition-all">
                    ...
                    <button onClick={() => handleDeleteCard(card.id)} disabled={isLoading} className="text-slate-500 hover:text-red-500 transition-colors p-2 bg-slate-100 rounded-lg hover:bg-red-100" title="Remove Card">
                      <X size={16} />
                    </button>
                  </div>
                ))}
              ...
              <div className="text-center py-12 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                ...
                <p className="text-xs text-slate-500 max-w-sm mx-auto">Link a debit card or bank account to enable secure withdrawals from your DEUS balance.</p>
              </div>
            )}
            <div className="mt-8 p-4 bg-blue-50 rounded-2xl border border-blue-200 flex items-start gap-3">
        ...
        {subTab === 'ACCESSIBILITY' && (
          <div className="space-y-8 max-w-2xl animate-in fade-in zoom-in-95 duration-300">
            <div>
              ...
              <p className="text-xs text-slate-500">Customize your visual experience. Settings are synchronized across all your devices.</p>
            </div>
            <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
              ...
              <div className="p-6 border-b border-slate-100 flex items-center justify-between hover:bg-slate-50 transition-colors">
                <div>
                  <h4 className="text-sm font-bold text-slate-800">App Theme</h4>
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-1">Select visual mode</p>
                </div>
                <select
                  value={accessSettings.theme}
                  onChange={(e) => handleSaveAccessibility({ ...accessSettings, theme: e.target.value })}
                  className="bg-slate-100 border border-slate-200 text-sm font-bold text-slate-800 rounded-xl px-4 py-2 outline-none focus:ring-2 focus:ring-blue-500"
                >
              ...
              <div className="p-6 border-b border-slate-100 flex items-center justify-between hover:bg-slate-50 transition-colors">
                <div>
                  <h4 className="text-sm font-bold text-slate-800">Adjust Text Size</h4>
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-1">Make text larger for better readability</p>
                </div>
                <select
                  value={accessSettings.textSize}
                  onChange={(e) => handleSaveAccessibility({ ...accessSettings, textSize: e.target.value })}
                  className="bg-slate-100 border border-slate-200 text-sm font-bold text-slate-800 rounded-xl px-4 py-2 outline-none focus:ring-2 focus:ring-blue-500"
                >
              ...
              {/* Color Contrast */}
              <div className="p-6 border-b border-slate-100 flex items-center justify-between hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => handleSaveAccessibility({ ...accessSettings, contrast: !accessSettings.contrast })}>
                <div>
                  <h4 className="text-sm font-bold text-slate-800">Increase Color Contrast</h4>
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-1">Darken borders and text elements</p>
                </div>
                <div className={`w-12 h-6 rounded-full transition-colors relative ${accessSettings.contrast ? 'bg-blue-600' : 'bg-slate-300'}`}>
                  <div className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-transform ${accessSettings.contrast ? 'translate-x-6' : ''}`}></div>
                </div>
              </div>
              {/* Reduce Motion */}
              <div className="p-6 flex items-center justify-between hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => handleSaveAccessibility({ ...accessSettings, motion: !accessSettings.motion })}>
                <div>
                  <h4 className="text-sm font-bold text-slate-800">Reduce Motion</h4>
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-1">Minimize animations and transition effects</p>
                </div>
                <div className={`w-12 h-6 rounded-full transition-colors relative ${accessSettings.motion ? 'bg-blue-600' : 'bg-slate-300'}`}>
                  <div className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-transform ${accessSettings.motion ? 'translate-x-6' : ''}`}></div>
                </div>
              </div>
            </div>
            {/* Screen Reader Info Box */}
            <div className="bg-blue-50 border border-blue-100 p-5 rounded-2xl flex items-start gap-4">
              <Accessibility className="text-blue-600 mt-1 flex-shrink-0" size={20} />
              <div>
                <h4 className="text-sm font-bold text-slate-800">Screen Reader Optimization</h4>
                <p className="text-[11px] font-medium text-slate-600 mt-1 leading-relaxed">
                  DEUS is automatically optimized for system-level screen readers (VoiceOver & TalkBack). Audio descriptions are natively applied to all visual elements and charts.
                </p>
              </div>
            </div>
          </div>
        ...
        {subTab === 'SECURITY' && (
          <div className="text-center py-12 text-slate-500">
            ...
            <p className="font-medium text-lg text-slate-500">Advanced Security configuration coming soon.</p>
          </div>
        )}
        {subTab === 'ABOUT' && (
          <div className="text-center py-12 text-slate-500 space-y-4">
            <Info size={48} className="mx-auto mb-4 opacity-50 text-blue-500" />
            <p className="font-medium text-lg max-w-md mx-auto text-slate-800">DEUS is your all-in-one financial platform designed to provide secure, intelligent, and supportive financial management.</p>
